...
export default function WorkListScreen({ folder, onBack, onSelect, onToggleCompleted, onAdd, onEdit, onDelete, onReorder }: Props) {
  ...
  const [isDragOver, setIsDragOver] = useState(false); // ファイルドロップ用
  ...
  // 並び替えモード時はfolder.worksそのままの順で表示、通常時はソート
    ...
    const onMove = (me: MouseEvent) => {
      ...
      for (const el of els) {
        ...
        if (me.clientY >= rect.top && me.clientY <= rect.bottom) {
          const hoverId = (el as HTMLElement).dataset.workId!;
        ...
        {sortedFiltered.length === 0 ? (
          ...
          <div className="space-y-2">
            {sortedFiltered.map((work) => {
              ...
              const isDragOverThis = dragOverId === work.id;
              return (
                <div
                  key={work.id}
                  data-work-id={work.id}
                  className={`relative transition-all duration-150 ${isDraggingThis ? "opacity-40 scale-[0.98]" : ""} ${isDragOverThis ? "ring-2 ring-[#7aa2f7] rounded-2xl" : ""}`}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
